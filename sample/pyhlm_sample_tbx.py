...
writer = tbx.SummaryWriter('/work/log/exp-1')

#%% Save init params
save_params(0, model)
save_loglikelihood(model)
writer.add_scalar('model/log_likelihood', model.log_likelihood(), 0)
...
for t in trange(train_iter):
    ...
    writer.add_scalar('model/log_likelihood', model.log_likelihood(), t + 1)
    writer.add_scalar('model/resample_model_time', resample_model_time, t + 1)
    writer.add_scalar('word/num_states', model.params['num_states'], t + 1)
    writer.add_scalar('word/word_length', model.params['word_length']['lmbda'], t + 1)
    writer.add_image('word/bigram_trans_matrix', np.expand_dims(model.params['bigram']['trans_matrix'], 0), t + 1)
    writer.add_scalar('letter/num_states', model.params['letter_hsmm']['num_states'], t + 1)
    writer.add_image('letter/bigram_trans_matrix', np.expand_dims(model.params['letter_hsmm']['bigram']['trans_matrix'], 0), t + 1)
